...
class Solver {
  ...
  static fix(value, idx) {
    ...
    else if (idx === 4 || idx === 5) max = 200; // brightness or contrast
    if (idx === 3) { // hue-rotate
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const real = document.querySelector(".realPixel");
  ...
  btn.addEventListener("click", () => {
    const target = hexToRgb(input.value);
    const solver = new Solver(target, new Color(0, 0, 0));
    const result = solver.solve();
    real.style.backgroundColor = target.toString();
    filterPixel.style.filter = result.filter;
    filterDetail.textContent = result.filter;
    lossDetail.textContent = `loss: ${result.loss}`;
  });
});
